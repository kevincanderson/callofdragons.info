...
fetch("../heroes.json")
    ...
    .then((data) => {
        ...
        const heroNameWindow = window.location.href.split("heroes/")[1];
        const heroPage = heroNameWindow.slice(0, heroNameWindow.length - 1);
        console.log(heroPage);
        data.filter((hero) => {
            const heroName = hero.name.toLowerCase();
            if (heroName === heroPage) {
              console.log(hero);
                heroInfo.innerHTML +=`<div class="hero-info-container">
